  ...
  return (
    <div className="p-4 sm:p-6 text-sm text-gray-900 max-w-[600px] mx-auto">
      <div className="mb-6">
        ...
        <h1 className="text-lg font-light tracking-[0.2em] uppercase">{personalInfo.fullName || t("yourName")}</h1>
      ...
      {experience.length > 0 && (
        <div className="mb-5">
          <h2 className="text-[10px] font-semibold tracking-[0.15em] uppercase mb-3" style={{ color: accentColor }}>{t("experience")}</h2>
          {experience.map((exp) => (
            <div key={exp.id} className="mb-3">
              <div className="flex justify-between items-baseline">
                <p className="text-xs font-medium">{exp.jobTitle}</p>
                <p className="text-[10px] text-gray-400">{exp.startDate} – {exp.current ? t("present") : exp.endDate}</p>
              </div>
      ...
      {skills.length > 0 && (
        <div className="mb-5">
          <h2 className="text-[10px] font-semibold tracking-[0.15em] uppercase mb-2" style={{ color: accentColor }}>{t("skills")}</h2>
      ...
      {projects.length > 0 && (
        <div className="mb-5">
          <h2 className="text-[10px] font-semibold tracking-[0.15em] uppercase mb-3" style={{ color: accentColor }}>{t("projects")}</h2>
          {projects.map((p) => (
