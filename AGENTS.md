...
- **PDF export is print-based**: the Download PDF button calls `printResumeFitToOnePage()` (`lib/print-utils.ts`) — it measures `.print-resume` offscreen, auto-scales via inline `zoom` if content exceeds one A4 page (~1123px at 794px width), shows an info toast, and resets zoom on `afterprint`. The printable copy lives in a `.print-resume` element (hidden with `display:none`, revealed by `@media print` rules in `app/globals.css`). Browser uses `document.title` as the PDF filename, so it is set from the resume title. Any change to preview rendering may need the same change mirrored in the print copy (`BuilderLayout.tsx`, `ShareResumeView.tsx`).
