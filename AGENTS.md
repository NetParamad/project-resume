...
- **PDF export is print-based**: the Download PDF button calls `printResumeFitToOnePage()` (`lib/print-utils.ts`) — it measures the offscreen print copy, auto-scales via inline `zoom` (iterative, max 4 steps), and prints. The printable copy is rendered through `PrintResumePortal` (`components/preview/PrintResumePortal.tsx`, used by BOTH `BuilderLayout.tsx` and `ShareResumeView.tsx`) into a `document.body`-level `.print-root.print-resume` div; `@media print` hides every other body child so page count depends only on the resume, and a fixed 794×1122 `overflow:hidden` box guarantees exactly one page (worst case clips). Browser uses `document.title` as the PDF filename, so it is set from the resume title. Toast outcomes: auto-scaled (info), too long / may clip (error, `pdfTooLongWarning`), zoom unsupported (error). Any change to preview rendering must be mirrored in the portal children of both call sites.
